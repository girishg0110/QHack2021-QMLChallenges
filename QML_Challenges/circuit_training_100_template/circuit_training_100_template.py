...
def optimize_circuit(params):
    ...
    def getGrad(circ, weights):
        s=1e-6
        gradient = np.zeros(NUM_PARAMETERS)
        gradQuantities = np.zeros((NUM_PARAMETERS, 2))
        for i in range(NUM_PARAMETERS):
            modWeight = weights
            modWeight[i] += s
            gradQuantities[i, 0] = circ(modWeight)
            modWeight[i] -= 2*s
            gradQuantities[i, 1] = circ(modWeight)
            
            gradient[i] = (gradQuantities[i, 0]-gradQuantities[i, 1])/(2*np.sin(s))
        return gradient
    ...
    while (iters < 50):
        ...
        if (optimal_value > new):
            quot = optimal_value/new
            optimal_value = new
            if (quot < 1+1e-3): break
        else: 
            params += lr * grad
            lr *= 0.5
        
        grad = getGrad(circuit, params)
        params -= lr * grad
        iters += 1
